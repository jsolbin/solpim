...
function AdminPage() {
  ...
  const [pendingArtworks, setPendingArtworks] = useState<ProtectedArtworkRecord[]>([])
      ...
      <Box component="main" sx={contentPageSx}>
        <Stack spacing={4} sx={contentContainerSx}>
          ...
          <Typography color="text.secondary">
            Review uploaded artworks and approve them to start the protection pipeline.
          </Typography>
          ...
          {isLoading ? <Typography>Loading pending artworks...</Typography> : null}
          ...
          <Stack spacing={2}>
            {pendingArtworks.map((artwork) => (
              ...
              >
                <Typography variant="h6">{artwork.title || 'Untitled artwork'}</Typography>
                <Typography color="text.secondary">Artwork ID: {artwork.id}</Typography>
                <Typography color="text.secondary">Owner: {artwork.ownerUid ?? 'Unknown'}</Typography>
                <Typography color="text.secondary">
                ...
                <Button
                  ...
                  variant="contained"
                >
                  {isApprovingArtworkId === artwork.id ? 'Approving...' : 'Approve'}
                </Button>
                <Button onClick={() => navigate(`/artworks/${artwork.id}`)} variant="outlined">
                  View detail
                </Button>
              </Stack>
            ))}
